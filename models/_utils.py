...
def kwonly_to_pos_or_kw(fn: Callable[..., D]) -> Callable[..., D]:
    ...
    try:
        ...
        raise TypeError(
            f"Found no keyword-only parameter on function '{fn.__name__}'"
        ) from None
    ...
    **weights: Tuple[str, Union[Optional[W], Callable[[Dict[str, Any]], Optional[W]]]]
    ...
    def outer_wrapper(builder: Callable[..., M]) -> Callable[..., M]:
        ...
        def inner_wrapper(*args: Any, **kwargs: Any) -> M:
            for (
                weights_param,
                (pretrained_param, default),
            ) in weights.items():  # type: ignore[union-attr]
                ...
                if pretrained_positional:
                    ...
                    kwargs[pretrained_param] = pretrained_arg = kwargs.pop(
                        weights_param
                    )
                else:
                ...
                if pretrained_arg:
                    default_weights_arg = (
                        default(kwargs) if callable(default) else default
                    )
                    if not isinstance(default_weights_arg, WeightsEnum):
                        raise ValueError(
                            f"No weights available for model {builder.__name__}"
                        )
                else:
